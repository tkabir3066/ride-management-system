...
const addDriverInfo = async (payload: IDriver, userId: string) => {
...
const getCompletedRides = async (driverId: string) => {
...
const updateAvailabilityToOnline = async (userId: string) => {
  const driver = await Driver.findOne({ userId });
  if (!driver) {
    throw new AppError(StatusCodes.BAD_REQUEST, "Driver not found");
  }
...
const updateAvailabilityToOffline = async (userId: string) => {
  const driver = await Driver.findOne({ userId });
  if (!driver) {
    throw new AppError(StatusCodes.BAD_REQUEST, "Driver not found");
  }
